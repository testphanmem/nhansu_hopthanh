...
const uiMinhChung = `
<style>
  /* Cấu trúc Modal chuẩn an toàn, Z-index 105055 nổi đè mọi form */
  ...
  <div class="up-container">
    ...
    <div class="up-box">
      ...
      <div id="linkInputAreaCV" style="display:none; text-align: left;">
          <input type="text" id="txtDirectLinkCV" placeholder="Dán đường link (Google Drive hoặc link web trực tiếp...) vào đây..." style="width:100%; padding:10px; border:1px solid #ccc; border-radius:6px; font-family: Arial, sans-serif;" oninput="checkDirectLinkCV()" autocomplete="off" spellcheck="false">
          <div style="font-size:12px; color:#666; margin-top:5px; font-style: italic;">* Lưu ý: Hỗ trợ link Google Drive đã mở quyền hoặc link web trực tiếp.</div>
      </div>
    </div>
...
// Chèn nguyên bản UI vào thân tài liệu web tĩnh một cách tự động
...
function SCV_OpenUploadModal(btnElement, currentLink, rootId, onlyLink = false) {
    if (typeof GLOBAL_PERM !== 'undefined' && !GLOBAL_PERM) { alert('⚠️ Không có quyền thao tác!'); return; }
    ...
    // NÂNG CẤP CHẾ ĐỘ CHỈ GÁN LINK KHI GỌI TỪ TAB ĐÁNH GIÁ
    if(onlyLink) {
...
function loadFoldersCV() {
   ...
   if(typeof google !== 'undefined' && typeof google.script !== 'undefined') {
       ...
       s.innerHTML = `<option value="${CURRENT_ROOT_ID}">📂 THƯ MỤC GỐC (Chỉ hiển thị Link)</option>`;
       s.disabled = false;
   }
}
...
function doUploadCV() {
   ...
   if (mode === 'link') {
       ...
       if (wMode === 'append' && LINK_CAN_XOA_CV && LINK_CAN_XOA_CV.trim() !== '') {
           ...
           setTimeout(() => { handleUpResultCV({ success: true, fileUrl: finalLinkStr, message: 'Đã chèn thêm link thành công!' }, st, btn); }, 300);
       } 
           ...
           setTimeout(() => { handleUpResultCV({ success: true, fileUrl: finalLinkStr, message: 'Đã thay thế và gán link mới thành công!' }, st, btn); }, 300);
       } 
       else {
           st.innerText = '⏳ Đang cập nhật và ghi đè đường dẫn...';
           setTimeout(() => { handleUpResultCV({ success: true, fileUrl: finalLinkStr, message: 'Đã gán link thành công!' }, st, btn); }, 300); 
       }
       return;
   }
...
function handleUpResultCV(res, st, btn) {
   if (res.success) { 
       ...
       if (GLOBAL_BTN_TRIGGER) {
           ...
           if (typeof veGhimModal === 'function') { veGhimModal(res.fileUrl); }
       }
